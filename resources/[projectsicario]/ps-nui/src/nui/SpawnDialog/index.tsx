...
const SpawnDialog: React.FC = () => {
  ...
  useNuiEvent<boolean>('nui-spawndialog', (data) => {
    if(data == true){
  ...
  function getSpawnLocations() {
    fetchNui("GetSpawnLocations", "ps-spawn").then(retData => {
      ...
      <div className="spawns">
        ...
        <div className="buttons" id="locations">
          {spawnLocations.map((location, index) => {
            return(<button onClick={() => {Selected(spawnLocations[index]);}} id={spawnLocations[index].id} style={{marginBottom: "20px", width: "100%", color: "rgb(220, 220, 220)", borderColor: "#23ACFF", border: "2px #23ACFF solid"}}>{spawnLocations[index].location}</button>)
          })}
          <button onClick={Submit} style={{ backgroundColor: "#31C031", width: "100%" }} id="submit">Continue</button>
        </div>
      </div>
    </div>
  );
